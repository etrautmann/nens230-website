%% Statistics and Hypothesis Testing Matlab Tutorial
% 
% This matlab tutorial is intended to show how to:
%
...
%
% Questions at the end of each of the 7 sections. Please type them up in
% a text file for submission.
%
% Eddy Albarran, modified from Franco Pestilli's Psych216A course
...
k = 10000;     % Choose the population size
N = randn(k,1);% Create the population, we use randn.m to sample from 
               % a gaussian distrbution with mean 0 and sd 1
...
for ii = 1:length(n)
 ...
 % size.
 sa = randsample(N,n(ii)); 
 sb = randsample(N,n(ii));
...
for jj = 1:length(n)
 ...
 sa = zeros(k,n(jj)); % zeros.m allows to preallocate memory
 sb = sa;
 for ii = 1:k
  sa(ii,:) = randsample(N,n(jj));
  sb(ii,:) = randsample(N,n(jj));
 end
...
xlabel('Difference between two random samples')
...
for ii = 1:length(n)
 plot([d_empirical(ii), d_empirical(ii)],[0 .2],'-','Color',color{ii},'LineWidth',2);
end

% Q1:
% Looking at the three lines generated, say something about how the
% emperical differences relate to the sample size. Does looking at this
% figure provide any insight to significance testing and the magic 0.05
% number?
...
%
% Here we show how to use bootstrap to generate the equivalent of a
% t-test.
...
title('Simulated true distribution (Chisquare, 6 DOF + Gaussian)')   % we add a title.
...
k = 10000; % number of times we will resample
sa_rand = zeros(n,k);
sb_rand = zeros(m,k);
for ii = 1:k
 temp = Shuffle(sh0);
 sa_rand(:,ii) = temp(1:n);
 sb_rand(:,ii) = temp(n+1:end);
 
 %sa_rand(:,ii) = randsample(sh0,n);
 %sb_rand(:,ii) = randsample(sh0,m);
end
...
text(d_empirical,y(2)*.95,sprintf('H0 is true with %2.2f probability.',p))
...
text(d_empirical,y(2)*0.95,txt)

% The following lines show how to set up a parametric t-test using matlab.
% This is out of scope for the present tutorial. 
% [h,p] = ttest2(sa,sb);
% disp(sprintf('\nH0 is true with %2.2f probability\n',p))

% Q2:
% Come up with a data set in which parametric and non-parametric tests
...
%
% So suppose we have one condition and measure not just one quantity, but 
% measure two distinct quantities. For example, suppose we measure both 
% the heights and weights of male adults.
% 
% What can we do with the data? 

% Let's generate two samples representing two variables drawn from gaussian
...
% Q3.
% Can you think of another way to use matlab to generate two correlated
% samples? (feel free to use the internet!)
...
% indicates a perfect positive linear relationship. (Note that we use
% Pearson's product-moment correlation, but there are other variants of
% correlation.) 
...
% n - 1. That is why we write std(v1,1) instea of simply std(v1).
...
% Q4.
% How would you generate s2 such that the correlation between it and s1 is
% -0.8 instead?
...
% Q5.
% How would implementing a non-bootstrap method change things?
...
% Q6.
% In the loop on line 472, why are s1 and s2 sampled independently?
...
k = 10000; % We set the number of montecarlo simulations
n = 30;    % The sample size
s1_sim = randn(k,n); % siulated distribution of samples
s2_sim = randn(k,n); % simulated distribution of samples
...
% were run.  Because value can be larger or smaller than the mean we
% compute the absolute value of the ditribution and r values.
p = sum(abs(r_empirical) < abs(r_dist))/k;
text(-0.8,y(2)*.95,sprintf('H0 is false with %2.4f probability.',p))

% Q7.
% What does this test tell us? How is this simulation different than that
% in Q6?
